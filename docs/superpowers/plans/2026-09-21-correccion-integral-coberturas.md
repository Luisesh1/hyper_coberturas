...
## Fase 0 · Contención — CERRADA (2026-09-21)

✅ **El usuario cerró manualmente todos los LPs y todas las protecciones el 2026-09-21.** Eran pruebas con capital real; la contención se resolvió fuera de banda y no queda nada que desmontar por código.

Consecuencias para el resto del plan, que conviene tener presentes:

- **Las fases 1–5 se construyen con la flota vacía**, que es el mejor momento posible: la migración `026_hedge_alerts` y cualquier cambio de política entran sin protecciones vivas encima.
- **La Fase 6 necesita protecciones nuevas.** La línea base de *Estado medido* queda como registro histórico del episodio, no como punto de partida medible: habrá que reconstruirla cuando vuelva a haber flota.
- **pp24/pp26/pp27 dejan de ser objetivos de intervención** y pasan a ser el caso de prueba documentado. Sus cifras siguen siendo válidas como fixtures — los tests de `delta-neutral-naked-exposure` y `delta-neutral-decision-log-truth` las usan literales.
...
- [x] **2.1 · Separar diagnóstico de acción en el log de decisiones.** ✅ **CUBIERTA POR 2.2 — sin migración.** El propósito declarado era «registrar ambas: qué se diagnosticó y qué se ejecutó». Con `execution_skipped_because` ya veraz, `decision <> 'hold' AND execution_skipped_because IS NULL` identifica exactamente las ejecuciones, y `decision`/`reason` conservan el diagnóstico. Una columna dedicada añadiría claridad nominal, no información: se descarta para no pagar una migración por semántica. Reabrir sólo si el análisis recurrente demuestra que la consulta se presta a error. Contexto original: `resolveRebalanceDecision` (`protected-pool-delta-neutral.helpers.js:504`) devuelve `rebalance_full` mirando sólo drift-vs-banda; no sabe nada del temporizador. Lo que gatea la ejecución es `effectiveShouldRebalance` (`evaluate.js:996`, `:1200`), otra variable. Registrar ambas: qué se diagnosticó y qué se ejecutó.
...
- [x] **2.5 · Segundo canal que no dependa de Telegram.** ✅ **HECHO.** Migración `026_hedge_alerts` + `hedge-alerts.repository.js`; el episodio se persiste **antes** de intentar el envío, así que la evidencia sobrevive a un canal caído. Expuesto en `/api/health/ready` (`checks.hedgeCoverage` + bloque `hedge`) y como gauges Prometheus en `/api/health/metrics` (`hedge_open_alert_episodes*`) — ese es el canal que de verdad no depende de Telegram. **El 200/503 no cambia**: ver *Riesgos*. Hereda la 1.3. Persistir alertas críticas en tabla y exponer el estado en `/api/health`, de modo que una protección con notional desnudo **degrade el health check**. Usar un campo de severidad en la respuesta, no un 503 seco (ver *Riesgos*).
